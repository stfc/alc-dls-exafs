...
def _parse_task_outputs(
    task: FeffTask,
    threshold: float = 5.0,
    store_paths: bool = True,
) -> tuple[np.ndarray | None, np.ndarray | None, list[PathResult]]:
    """Parse chi.dat and path files for a single completed task."""
...
class BatchExecutor:
    ...
    def run(self) -> Path:
        ...
        k_reference = (
            self.k_grid if self.k_grid is not None else np.arange(0.05, 20.0, 0.05)
        )
        ...
        with BatchShardWriter(
            ...
            for chunk_start in range(0, total_tasks, self.chunk_size):
                ...
                for t in chunk:
                    k_res, chi_res, paths = _parse_task_outputs(
                        t, threshold=self.threshold, store_paths=self.store_paths
                    )

                    if k_res is not None and chi_res is not None:
                        # Re-grid chi onto k_reference
                        chi_interp = np.interp(
                            k_reference, k_res, chi_res, left=0.0, right=0.0
                        )
                    else:
                        chi_interp = np.zeros_like(k_reference)

                    writer.add_task_result(
                        frame_idx=t.frame_idx,
                        site_idx=t.site_idx,
                        absorber_element=t.absorber_element,
                        chi=chi_interp,
                        paths=paths if self.store_paths else None,
                    )
