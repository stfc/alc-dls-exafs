...
def validate_absorber(atoms: Atoms, absorber: Union[str, int]) -> str:
    ...
    if isinstance(absorber, int):
        ...
        if absorber_element not in symbols:
            raise ValueError(f"Absorber element {absorber_element} not found in structure")
...
def generate_pymatgen_input(atoms: Atoms, absorber: Union[str, int], output_dir: Path, config) -> Path:
...
def generate_larixite_input(atoms: Atoms, absorber: Union[str, int], output_dir: Path, config) -> Path:
    ...
    with tempfile.NamedTemporaryFile(suffix=".cif", prefix="larch_temp_", delete=False) as tmp:
        ...
        try:
            ...
            if config.user_tag_settings:
                ...
                present_tags = {line.split()[0] for line in modified_lines if line.strip() and not line.startswith('*')}
                for tag, value in config.user_tag_settings.items():
...
def run_feff_calculation(feff_dir: Path, verbose: bool = False) -> bool:
    ...
    try:
        ...
        if verbose:
            ...
            try:
            ...
            except Exception as feff_error:
                ...
                try:
                    with open(log_path, 'a', encoding='utf-8', errors='replace') as log_file:
                        ...
                        log_file.write(f"FEFF result (without output capture): {result}\n")
                except:
                    pass
        ...
        try:
            with open(log_path, 'a', encoding='utf-8', errors='replace') as log_file:
                ...
                output_files = [f for f in feff_files if f.name not in ['feff.inp', 'feff.log']]
